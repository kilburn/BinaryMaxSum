 ...
 * Max-sum "Two-sided reserve" factor.
  * 
 * Given two sets of neighbors A = {a_1, ..., a_p} and B = {b_1, ..., b_q},
 * this factor tries to ensure that there are at least the same number of 
 * neighbors from the first set chosen than from the second one. 
 * That is, f(a_1, ..., a_p, b_1, ..., b_q) = 0 if (\sum_i a_i) >= (\sum_j b_j) 
 * or (-)\infty otherwise.
 * <p/>
...
public class TwoSidedReserveFactor<T> extends AbstractFactor<T> {
	...
	protected long iter() {
		...
		final double nuAtheta = (theta == 0) ? -op.getWorstValue() : setAPairs.get(theta-1).value;
		final double nuAthetaPlus = (nElementsA > theta) ? setAPairs.get(theta).value : op.getWorstValue();
		final double nuBtheta = (theta == 0) ? -op.getWorstValue() : setBPairs.get(theta-1).value;
		final double nuBTthetaPlus = (nElementsB > theta) ? setBPairs.get(theta).value : op.getWorstValue();
	...
	private int getTheta(List<Pair> setAPairs, List<Pair> setBPairs) {
		...
		while(theta < n
				...
				&& op.compare(setBPairs.get(theta).value + setAPairs.get(theta).value, 0) >= 0) {
			theta++;
		}
...
//		while(nPositive < pairsLength && pairs.get(nPositive).value >= 0) {
		int nPositive = 0;
		while(nPositive < pairsLength && op.compare(pairs.get(nPositive).value, 0) >= 0) {
			nPositive++;
		}
